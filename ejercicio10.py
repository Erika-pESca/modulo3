...
def explorar_estructura(data, nivel=1):
    if isinstance(data, dict):
        for k, v in data.items():
            resultados.append((k, nivel, "dict key"))
            explorar_estructura(v, nivel + 1)
    elif isinstance(data, (list, tuple, set)):
        for item in data:
            explorar_estructura(item, nivel + 1)
    else:
        resultados.append((data, nivel, type(data).__name__))
...
def test_explorar_lista_simple():
    ...
    assert len(resultados) == 3  # noqa: PLR2004
...
def test_profundiad_correcta_en_anidacion():
    ...
    assert max(profundidades) > 3  # noqa: PLR2004
